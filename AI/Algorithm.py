...
exemple2.append(character("Arti Ficelle","0",True,391,35,3,(5,3)))
...
def IsCellFree(wanted_cell,char_list):
    available = True
...
def TheoricalMovment(starting_pos,char_list,mouvment,n = 4):
    if (n != 0) :
        if(-1<starting_pos[0]<7 and -1<starting_pos[1]<7):
            if(IsCellFree(starting_pos,exemple)):
                mouvment[starting_pos[0]][starting_pos[1]] = 1
                TheoricalMovment((starting_pos[0]+1,starting_pos[1]),char_list,mouvment,n-1)
                TheoricalMovment((starting_pos[0]-1,starting_pos[1]),char_list,mouvment,n-1)
                TheoricalMovment((starting_pos[0],starting_pos[1]+1),char_list,mouvment,n-1)
                TheoricalMovment((starting_pos[0],starting_pos[1]-1),char_list,mouvment,n-1)
            else:
...
TheoricalMovment(exemple2[0].position,exemple2,mouvment)
